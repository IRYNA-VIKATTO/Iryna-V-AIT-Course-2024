...
//                }
    ...
    public int lastIndexOf(Object o) {
        int index = size - 1; // Начнем с последнего индекса
        if (o != null) { // Если ищем ненулевой элемент
            for (Node<E> node = last; node != null; node = node.prev, index--) {
                if (o.equals(node.data)) { // Сравниваем данные узла с искомым объектом
                    return index; // Если нашли совпадение, возвращаем индекс
                }
            }
        } else { // Если ищем null
            for (Node<E> node = last; node != null; node = node.prev, index--) {
                if (node.data == null) { // Если данные в узле равны null
